...
def find_argumets_and_parameterize(path, logger, mode, method_parameterization_args, num_of_parameterized_mol_args):
    ...
    if mode == "parameterization_find_args":
        ...
        if method_parameterization_args:
            ...
            method_parameterization = "combined"
        command = "python3 calculator_charges.py --mode parameterization --method {} --method_parameterization {} --parameters {} --sdf_input {} --right_charges {} " \
                  " --new_parameters {} --chg_output {} -f -v --make_html --save_fig > output.txt 2>&1"\
            .format(method, method_parameterization, parameters.split("/")[-1], sdf_file.split("/")[-1], right_charges.split("/")[-1], new_parameters, chg_output)
        system("./modules/send_meta.sh {} {} {} '{}'".format(parameters, sdf_file, right_charges, command))
